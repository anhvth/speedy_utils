...
from typing import Any, Dict, List, Type, get_type_hints, Annotated, get_origin, get_args, cast, Union
    ...
    def __new__(cls, name, bases, namespace, **kwargs):
        ...
        for field_name, field_type in annotations.items():
            ...
            if field_desc and field_desc.field_type == "input":
                input_fields[field_name] = {"type": field_type, "desc": field_desc.desc, **field_desc.kwargs}
            elif field_desc and field_desc.field_type == "output":
                output_fields[field_name] = {"type": field_type, "desc": field_desc.desc, **field_desc.kwargs}
...
class Signature(metaclass=SignatureMeta):
    ...
    def get_input_model(cls) -> Union[Type[BaseModel], Type[str]]:
        """Generate Pydantic input model from input fields, or return str if no input fields."""
        if not cls._input_fields:
            # Return str type to indicate string-only input
            return str
        ...
        for field_name, field_info in cls._input_fields.items():
            ...
            field_kwargs = {k: v for k, v in field_info.items() if k not in ["type", "desc"]}
            if desc:
        ...
        input_model = type(f"{cls.__name__}Input", (BaseModel,), {"__annotations__": annotations, **fields})
    ...
    def get_output_model(cls) -> Type[BaseModel]:
        ...
        # Create dynamic Pydantic model
        output_model = type(f"{cls.__name__}Output", (BaseModel,), {"__annotations__": annotations, **fields})
...
if __name__ == "__main__":
    ...
    for judge_class in [FactJudge, FactJudgeOldSyntax]:
        ...
        input_text = judge.format_input(
            context="The sky is blue during daytime.", question="What color is the sky?", answer="Blue"
        )
        print("\nFormatted Input:")
        print(input_text)
