...
def main(argv: List[str] | None = None) -> int:
    ...
    parser.add_argument("--min-sec", type=float, default=0.2, help="Minimum seconds to show")
    parser.add_argument("--no-x", action="store_true", help="Don't try -X importtime")
    parser.add_argument("--raw", action="store_true", help="Show raw -X output in addition")
        ...
        if not args.no_x:
            ...
            if ok:
                ...
                if filtered:
                    ...
                    print(f"No top-level modules >= {args.min_sec:.3f}s (from -X importtime)")
                if args.raw:
