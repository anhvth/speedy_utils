...
# Monkey-patch dill.load to be dill.loads so that the multiprocessing initializer works
import dill
...
from speedy_utils.multi_worker.thread import RunErr  # Import the custom exception
...
def test_multi_thread_error():
    ...
    try:
        ...
        # Note: The error is wrapped in a RunErr, so we catch that.
        multi_thread(
            func=error_func,
            orig_inputs=range(5),
            workers=2,
...
def test_multi_process_success():
    ...
    # Use functools.partial instead of a lambda (so that the function is pickleable)
